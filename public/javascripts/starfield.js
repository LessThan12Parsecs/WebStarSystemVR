...
AFRAME.registerComponent('star-system', {
  ...
  update: function() {
    ...
    console.log(cluster);
    while (stars.vertices.length < cluster.length) {
        stars.vertices.push(new THREE.Vector3(cluster[i].x,cluster[i].y,cluster[i].z));
        i++;
        console.log(new THREE.Vector3(cluster[i].x,cluster[i].y,cluster[i].z))
    }
